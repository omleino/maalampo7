# app.py — Lämmitysvaihtoehtojen vertailu PDF-raportilla (horisontti = laina-aika + 1)
...
def luo_pdf(kaavio, tbl_df, pb1, pb2, pb3, lainaosuus, syotteet, vuodet_teksti):
    """
    Luo PDF-raportin. Otsikot ja tekstisisältö käyttävät dynaamista vuosihorisonttia.
    """
    buffer = BytesIO()
    ...
    if tbl_df is not None and len(tbl_df) > 0:
        taulu = [tbl_df.reset_index().columns.to_list()] + tbl_df.reset_index().values.tolist()
        table = Table(taulu)
        ...
        elements.append(Paragraph("Taulukkoon ei mahtunut rivejä tällä aikajänteellä.", styles['Normal']))
    elements.append(Spacer(1, 0.2 * inch))
...
# Vastiketaulukko (5 v välein). Jos horisontti <5, näytetään viimeinen vuosi.
...
# Takaisinmaksuaika (lasketaan dynaamisella horisontilla)
...
pdf = luo_pdf(fig, tbl, pb1, pb2, pb3, lainaosuus, syotteet, vuodet_teksti)
